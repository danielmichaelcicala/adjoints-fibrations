...
\begin{proof}
  Decompose $U$ into a Street opfibration $U' \from \X \to \A'$ followed by an equivalence $H \from \A' \to \A$ per Lemma \ref{lem:street_decomposed}.  If the $U$-fibres have have $\cat{J}$-colimits, then so do the equivalent $U'$-fibres.  By Lemma \ref{lem:fibrewiselimits}, $\X$ has $\cat{J}$-colimits and $U'$ strictly preserves them, which implies that $U$ also preserves them.  

  Conversely, because $U$ preserves $J$-colimits, the diagram
  \begin{equation*}
  \begin{tikzpicture}
    \node (X) at (0,2) {$ \X $};
    \node (A) at (0,0) {$ \A $};
    ...
    \node (AJ) at (2,0) {$ [J,\A] $};
    \draw [cd]
    (X) edge[] node[left]{$U$} (A)
    (XJ) edge[] node[right]{$[J,U]$} (AJ)
    (X) edge[] node[above]{$\Delta$} (XJ)
    (A) edge[yshift=0.3cm] node[above]{$\Delta$} (AJ)
    (AJ) edge[yshift=-0.3cm] node[below]{$\colim$} (A); % TODO fix these yshifts
    \node () at (1,0) {$ \bot $};
  \end{tikzpicture}
\end{equation*}
can be completed to 
\begin{equation*}
  \begin{tikzpicture}
    ...
    (X) edge[] node[left]{$U$} (A)
    (XJ) edge[] node[right]{$[J,U]$} (AJ)
    (X) edge[yshift=0.3cm] node[above]{$\Delta$} (XJ)
    (XJ) edge[yshift=0.3cm] node[below]{$\colim$} (X) 
    (A) edge[yshift=0.3cm] node[above]{$\Delta$} (AJ)
    (AJ) edge[yshift=-0.3cm] node[below]{$\colim$} (A); % TODO fix these yshifts
    \node () at (1,0) {$ \bot $};
    \node () at (1,2) {$ \bot $};
...
\end{equation*}
  ...
  \daniel{pick up here with 'only if' direction. Think: U preserves colimits implies U' strictly?  U' strictly even needed in grothendieck case? See hermida's paper and my marginal notes}
