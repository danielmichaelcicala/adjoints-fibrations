...
% TODO make commutative diagrams uniform
% TODO make \cref vs \ref
...
To show that $L$ is left adjoint to $\U$, it suffices to establish a natural bijection $\X(\L a,x)\cong\A(a,\U x)$ for any $a\in\A$, $x\in\X$. Indeed, each morphism in the total category $k\colon \L a\to x$ above $f=\U k$, factorizes uniquely (cf.~\cref{factor}) as
\begin{displaymath}
