...
The other direction of the statement is not of relevance for our purposes. Notice that in particular, since $\U$ has a left adjoint $L$, it preserves all limits that exist in $\D$; this makes the condition for its existence look unintuitive. 

The dual result states that a fibration has a right adjoint right inverse %still counit needs to be id
if and only if the fibers have terminal objects that are preserved by the reindexing functors. 
