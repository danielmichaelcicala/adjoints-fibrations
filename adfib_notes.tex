...
\item {\daniel For adjunctions, we've been using L and
    R. But that's only when we are fixing an adjunction.
    Here we're proving things are adjunctions, so maybe F
    and G?} {\chris again, I think U was kind of suggested
    for our central opfibration. I don't mind that
    tbh.}{\daniel I thought John likes L,R as
    adjoints. I'm happy to use U instead. }
     
\item {\daniel I like Joe's mathsf font for categories.
    I'll macro that up.}{\chris I think you mean Joe has
    been using mathsf for named categories? I don't remember
    anyone arguing about mathcal for general categories as
    is the norm, but again if you don't like it no prob. I'm
    pretty used to it I admit!}{\daniel Here's another
    misundertanding by me.  I though ALL categories were
    mathsf, not just the named ones.  You're right mathcal
    is super common and I'm happy to use that
    too. }
\item {\daniel Basically, for all of these notation questions, I
  ...
  talk in person just so I don't confuse anything further.}
%     {\daniel Yes, I'm happy to use small letters for objects.} {\chris Great!}
\end{itemize}
...
Dually, the functor $\U \colon \A \to \X$ is an \define{opfibration} if $\U^\mathrm{op}$ is a fibration, i.e.\ for every $c \in \A _x$ and $h \colon x \to y$ in $\X$, there is a cocartesian morphism with domain $c$ above $h$, the \define{cocartesian lifting} of $c$ along $h$ with the dual universal property:
...
If $P\colon \A \to\X$ is a fibration, assuming the axiom of choice we may select a cartesian arrow over each $f\colon x\to y$ in $\X$ and $b\in\A _y$, denoted by $\Cart(f,b)\colon f^*(b)\to b$. Such a choice of cartesian liftings is called a \define{cleavage} for $P$, which is then called a \define{cloven fibration}; any fibration is henceforth assumed to be cloven. Dually, if $\U$ is an opfibration, for any $c\in\A _x$ and $h \colon x \to y$ in $\X$ we can choose a cocartesian lifting of $c$ along $h$, $\Cocart(h,c)\colon c\longrightarrow h_!(c)$. 
